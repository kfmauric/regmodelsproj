...
###Introduction  
Starting from the mtcars dataset in R, an anlysis will be done to quamtify the difference between manual and automatic transimission on an population fo cars. The types of cars represented in the data set range from american muscle and luxury cars to exotic sports cars to small economy cars. As a starting point in the anaylsis, a few exploratory plots will be created to examine the relationships between the parameters of the data set.
...
summary(fit4)
a<-qplot(predict(fit4),resid(fit4))+geom_smooth(method="lm")
#plot(fit4)
```
